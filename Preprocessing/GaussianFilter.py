...
def apply_gaussian_filter(
    ...
    if color_space == "bgr":
        result = cv2.GaussianBlur(img, (kernel_size, kernel_size), sigmaX=sigma, sigmaY=sigma)
    elif color_space == "ycrcb":
        ycrcb = cv2.cvtColor(img, cv2.COLOR_BGR2YCrCb)
        ycrcb[:, :, 0] = cv2.GaussianBlur(ycrcb[:, :, 0], (kernel_size, kernel_size), sigmaX=sigma, sigmaY=sigma)
        result = cv2.cvtColor(ycrcb, cv2.COLOR_YCrCb2BGR)
    else:  # lab
        lab = cv2.cvtColor(img, cv2.COLOR_BGR2LAB)
        lab[:, :, 0] = cv2.GaussianBlur(lab[:, :, 0], (kernel_size, kernel_size), sigmaX=sigma, sigmaY=sigma)
