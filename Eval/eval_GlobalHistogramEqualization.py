...
from Preprocessing.GlobalHistogramEqualization import apply_global_histogram_equalization

VALID_EXTENSIONS = {".png", ".jpg", ".jpeg", ".bmp", ".tif", ".tiff", ".webp"}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluación de Ecualización Global con MLflow")
    parser.add_argument("--dataset_root", type=str, required=True, help="Ruta raíz del dataset. Debe contener carpetas por condición.")
    parser.add_argument("--mlruns_dir", type=str, required=True, help="Ruta donde MLflow almacenará los runs.")
    parser.add_argument("--experiment_name", type=str, default="global_histogram_equalization_experiment", help="Nombre del experimento en MLflow.")
    parser.add_argument("--output_dir", type=str, default="outputs_global_histogram_equalization", help="Directorio donde se guardan CSVs y previews.")
...
def build_parameter_grid() -> List[Dict]:
    return [
        {"color_space": color_space, "preserve_luminance": preserve_luminance}
        for color_space, preserve_luminance in itertools.product(["ycrcb", "lab"], [True, False])
    ]


def format_run_name(condition_name: str, color_space: str, preserve_luminance: bool) -> str:
    return f"{condition_name}__global_he__cs_{color_space}__pl_{str(preserve_luminance).lower()}"
...
def main() -> None:
    ...
    for condition_dir in condition_dirs:
        ...
        for cfg in parameter_grid:
            ...
            run_name = format_run_name(condition_name, color_space, preserve_luminance)
            ...
            with mlflow.start_run(run_name=run_name):
                ...
                mlflow.log_param("method", "global_he")
                ...
                for img_path in image_paths:
                    ...
                    processed_bgr = apply_global_histogram_equalization(
                        image_bgr=img_bgr,
                ...
                print(f"[OK] {run_name} | NIQE={summary['mean_niqe']:.4f} | BRISQUE={summary['mean_brisque']:.4f} | PIQE={summary['mean_piqe']:.4f}")

    if not all_run_rows:
        raise RuntimeError("No se generaron resultados. Verifica dataset y parámetros.")

    final_df = pd.DataFrame(all_run_rows).sort_values(by=["condition", "composite_score"], ascending=[True, True])
    final_csv = output_dir / "global_histogram_equalization_all_runs_summary.csv"
