...
from Preprocessing.GaussianFilter import apply_gaussian_filter
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluacion de filtro gaussiano con MLflow")
    parser.add_argument("--dataset_root", type=str, required=True)
    parser.add_argument("--mlruns_dir", type=str, required=True)
    parser.add_argument("--experiment_name", type=str, default="gaussian_filter_experiment")
    parser.add_argument("--output_dir", type=str, default="outputs_gaussian_filter")
...
def build_parameter_grid() -> List[Dict]:
    configs = []
    for kernel_size, sigma, color_space, preserve_luminance in itertools.product(
        [3, 5, 7],
        [0.5, 1.0, 1.5, 2.0],
        ["bgr", "ycrcb", "lab"],
        [True, False],
...
def main() -> None:
    ...
    for condition_dir in condition_dirs:
        ...
        for cfg in parameter_grid:
            run_name = f"{condition_dir.name}_k_{cfg['kernel_size']}_s_{str(cfg['sigma']).replace('.', '_')}_cs_{cfg['color_space']}_pl_{str(cfg['preserve_luminance']).lower()}"
            rows = []
            with mlflow.start_run(run_name=run_name):
                for k, v in cfg.items():
                    mlflow.log_param(k, v)
                mlflow.log_param("condition", condition_dir.name)
                mlflow.log_param("technique", "gaussian_filter")
                for img_path in image_paths:
                    img_bgr = cv2.imread(str(img_path), cv2.IMREAD_COLOR)
                    if img_bgr is None:
                        continue
                    processed_bgr = apply_gaussian_filter(
                        image_bgr=img_bgr,
    ...
    final_df.to_csv(output_dir / "gaussian_filter_all_runs_summary.csv", index=False)
    best_by_condition = final_df.groupby("condition", as_index=False).first()
    best_by_condition.to_csv(output_dir / "best_gaussian_filter_by_condition.csv", index=False)
