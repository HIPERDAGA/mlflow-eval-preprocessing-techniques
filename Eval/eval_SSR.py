...
from Preprocessing.SSR import apply_ssr

VALID_EXTENSIONS = {'.png', '.jpg', '.jpeg', '.bmp', '.tif', '.tiff', '.webp'}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Evaluación de Single Scale Retinex (SSR) con MLflow')
    ...
    parser.add_argument('--experiment_name', type=str, default='ssr_experiment', help='Nombre del experimento en MLflow.')
    parser.add_argument('--output_dir', type=str, default='outputs_ssr', help='Directorio donde se guardan CSVs y previews.')
...
def build_parameter_grid() -> List[Dict]:
    configs: List[Dict] = []
    for sigma, color_space, preserve_luminance in itertools.product(
        [15, 30, 80, 150, 250],
        ['bgr', 'ycrcb', 'lab'],
        [True, False],
    ):
        configs.append({'sigma': float(sigma), 'color_space': color_space, 'preserve_luminance': preserve_luminance})
    return configs


def format_run_name(condition_name: str, sigma: float, color_space: str, preserve_luminance: bool) -> str:
    sigma_tag = str(sigma).replace('.', '_')
    preserve_tag = str(preserve_luminance).lower()
    return f'{condition_name}_ssr_s_{sigma_tag}_cs_{color_space}_pl_{preserve_tag}'
...
def main() -> None:
    ...
    for condition_dir in condition_dirs:
        ...
        for cfg in parameter_grid:
            ...
            run_name = format_run_name(condition_name, sigma, color_space, preserve_luminance)
            per_image_rows = []
            preview_saved = False

            with mlflow.start_run(run_name=run_name):
                mlflow.log_param('condition', condition_name)
                mlflow.log_param('technique', 'ssr')
                ...
                for img_path in image_paths:
                    ...
                    metrics = compute_metrics_from_bgr(processed_bgr, niqe_metric=niqe_metric, brisque_metric=brisque_metric)
